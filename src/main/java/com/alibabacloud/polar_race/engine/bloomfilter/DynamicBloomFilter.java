...
public class DynamicBloomFilter implements BloomFilter {
    ...
    // 原本的BloomFilter是使用k个hash函数作用于输入数据。
    // 此处是采用把输入数据和0~k-1 这k个数组合在一起使用同一个hash函数计算k次。
    public void add2Slice(Slice key, Slice result, int bitSetSize) {
        ...
        for (int x = 0; x < k; x++) {
            ...
            int i = Math.abs((int) hash); // hash值可能是负数
            //slice代替bitset
            int index = i / 8;
            int offset = 7 - i % 8;
    ...
    public long createHash(byte[] data) {
        ...
        synchronized (digestFunction) { // TODO synchronized
